module.exports = {
    ...
    execute(msg) {
        ...
        if ((inputArr[0] == undefined) || (inputArr[1] == undefined) || (inputArr[2] == undefined) || (inputArr[3] == undefined) || (inputArr[4] == undefined) || (inputArr[5] == undefined) || (inputArr[6] == undefined)) {
            msg.channel.send(`**Error:** Make sure to include all information!\nex. \`!editinfo name age sex grade baptized state country\` - \`!editinfo daniel 14 m 9 yes Illinois USA\``);
        } else {
            db.prepare(`INSERT OR REPLACE INTO info VALUES(?, ?, ?, ?, ?, ?, ?);`).run(inputArr[0].toLowerCase(), inputArr[1], inputArr[2], inputArr[3], inputArr[4], inputArr[5], inputArr[6]);
            msg.channel.send(`**Info Changed**`);
        }
