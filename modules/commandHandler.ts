module.exports = {
    commands(client) {
        ...
        const otherCommandFiles = fs.readdirSync(`${commandDir}/fun`).filter(file => file.endsWith('.ts'));
        const infoCommandFiles = fs.readdirSync(`${commandDir}/info`).filter(file => file.endsWith('.ts'));
