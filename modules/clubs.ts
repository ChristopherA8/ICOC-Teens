module.exports = {
    async clubReactions(client, Discord) {
        ...
        const animefilter = (reaction, user) => user.id !== `761792910088994816` && reaction.emoji.name == `⛩️`;
        const artfilter = (reaction, user) => user.id !== `761792910088994816` && reaction.emoji.name == `🎨`;
        const athleticsfilter = (reaction, user) => user.id !== `761792910088994816` && reaction.emoji.name == `🏃‍♂️`;
        const bookfilter = (reaction, user) => user.id !== `761792910088994816` && reaction.emoji.name == `📚`;
        const dndfilter = (reaction, user) => user.id !== `761792910088994816` && reaction.emoji.name == `🎲`;
        const foodfilter = (reaction, user) => user.id !== `761792910088994816` && reaction.emoji.name == `🍜`;
        ...
        animecollector.on('collect', async (r, user) => {
            ...
            if (!animeClub.permissionOverwrites.find(mem => mem.id == reactionMember.id)) {
                animeClub.updateOverwrite(reactionMember, {
                    VIEW_CHANNEL: true
                }).catch(console.error);
            } else {
        ...
        artcollector.on('collect', async (r, user) => {
            ...
            if (!artClub.permissionOverwrites.find(mem => mem.id == reactionMember.id)) {
                artClub.updateOverwrite(reactionMember, {
                    VIEW_CHANNEL: true
                }).catch(console.error);
            } else {
        ...
        athleticcollector.on('collect', async (r, user) => {
            ...
            if (!athleticClub.permissionOverwrites.find(mem => mem.id == reactionMember.id)) {
                athleticClub.updateOverwrite(reactionMember, {
                    VIEW_CHANNEL: true
                }).catch(console.error);
            } else {
        ...
        bookcollector.on('collect', async (r, user) => {
            ...
            if (!bookClub.permissionOverwrites.find(mem => mem.id == reactionMember.id)) {
                bookClub.updateOverwrite(reactionMember, {
                    VIEW_CHANNEL: true
                }).catch(console.error);
            } else {
        ...
        dndcollector.on('collect', async (r, user) => {
            ...
            if (!dndClub.permissionOverwrites.find(mem => mem.id == reactionMember.id)) {
                dndClub.updateOverwrite(reactionMember, {
                    VIEW_CHANNEL: true
                }).catch(console.error);
            } else {
        ...
        foodcollector.on('collect', async (r, user) => {
            ...
            if (!foodClub.permissionOverwrites.find(mem => mem.id == reactionMember.id)) {
                foodClub.updateOverwrite(reactionMember, {
                    VIEW_CHANNEL: true
                }).catch(console.error);
            } else {
        ...
        gamingcollector.on('collect', async (r, user) => {
            ...
            if (!gamingClub.permissionOverwrites.find(mem => mem.id == reactionMember.id)) {
                gamingClub.updateOverwrite(reactionMember, {
                    VIEW_CHANNEL: true
                }).catch(console.error);
            } else {
        ...
        tvcollector.on('collect', async (r, user) => {
            ...
            if (!tvClub.permissionOverwrites.find(mem => mem.id == reactionMember.id)) {
                tvClub.updateOverwrite(reactionMember, {
                    VIEW_CHANNEL: true
                }).catch(console.error);
            } else {
        ...
        musiccollector.on('collect', async (r, user) => {
            ...
            if (!musicClub.permissionOverwrites.find(mem => mem.id == reactionMember.id)) {
                musicClub.updateOverwrite(reactionMember, {
                    VIEW_CHANNEL: true
                }).catch(console.error);
            } else {
