module.exports = {
    logging(client) {
        ...
        client.on('guildMemberRemove', async member => {
            ...
            if (!kickLog) return embed(channel, `${member.user.tag} left the guild`, member)
            ...
            if (target.id === member.id) {
                embed(channel, `${member.user.tag} left the guild.\nKicked by ${executor.tag}?`, member) //channel.send(`> ${member.user.tag} left the guild; kicked by ${executor.tag}? <:dead:765721212033695784>`);
            } else {
                embed(channel, `${member.user.tag} left the guild.\nAudit log fetch was inconclusive.`, member) //channel.send(`> ${member.user.tag} left the guild, audit log fetch was inconclusive. <:dead:765721212033695784><:dead:765721212033695784><:dead:765721212033695784>`);
            }
